//===----------------------------------------------------------------------===//
// VENDORED FROM LLVM -- DO NOT EDIT
//
// Source: llvm/lib/Transforms/Instrumentation/CFGMST.h
// Origin: llvm-project @ llvmorg-15-init-942-g1609282fe7fc
//
// CFGMST.h is an internal LLVM library header: it is not installed by an LLVM
// distribution, so it cannot be picked up from /usr/lib/llvm-15/include. It is
// vendored here so the MST placement arm binds to LLVM PGO's *real* spanning
// tree construction rather than a reimplementation (see docs/adr/0002 in the
// customPass tree). Verified to compile against stock LLVM 15.0.7 headers.
//
// If the LLVM version this project builds against changes, re-vendor this file
// from the matching release rather than editing it in place.
//===----------------------------------------------------------------------===//
